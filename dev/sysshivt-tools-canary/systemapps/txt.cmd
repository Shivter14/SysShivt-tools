...
set txt.boxY=%~5
:reload
if not defined txt.boxW if %sst.defaultresX% gtr 80 (set /a txt.boxW=%sst.crrresX%-12
) else set /a txt.boxW=%sst.crrresX%-2
if not defined txt.boxH if %sst.defaultresY% gtr 20 (set /a txt.boxH=%sst.crrresY%-6
) else set /a txt.boxH=%sst.crrresY%-2
if not defined txt.boxX set /a txt.boxX=%sst.defaultresX%/2-(%txt.boxW%/2-1)
if not defined txt.boxY set /a txt.boxY=%sst.defaultresY%/2-(%txt.boxH%/2)
set /a txt.boxY=%txt.boxY%+1
set /a txt.boxH=%txt.boxH%-1
...
set /a txt.con.ls=0
set /a txt.con.le=%txt.con.ls%+%txt.boxH%-1
:main
call sbox %txt.boxX% %txt.boxY% %txt.boxW% %txt.boxH% %sst.window.BGcolor%%sst.window.TIcolor%
call batbox /g %txt.boxX% %txt.boxY% /c 0x9f /a 32 /a 30 /d "  " /a 31 /a 32 /c 0xbf /d " Reload " /g %txt.eabX% %txt.boxY% /c 0xcf /d " X " /g %txt.menu% %txt.boxY% /c 0x%sst.window.TIcolor%%sst.window.BGcolor%
set txt.bar=
