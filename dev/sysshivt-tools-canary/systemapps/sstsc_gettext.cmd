...
if exist "%sst.temp%\sysscene\%sstsc.session%\INTERRUPT" (
	timeout 1 /nobreak > nul
	goto start
)
echo.>"%sst.temp%\sysscene\%sstsc.session%\INTERRUPT"
call batbox /c 0x%~2 /g %~3 %~4 /d "%gtxt.blank%" /g %~3 %~4 /d "%gtxt.input%"
call getinput
set gtxt.errorlevel=%errorlevel%
if exist "%sst.temp%\sstsc\%sstsc.session%\INTERRUPT" del /Q "%sst.temp%\sstsc\%sstsc.session%\INTERRUPT"
if "%gtxt.errorlevel%" equ "13" goto input
if "%gtxt.errorlevel%" equ "27" exit /b 1
