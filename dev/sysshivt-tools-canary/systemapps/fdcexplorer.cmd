...
set explorer.sftm=
if "%sst.errorlevel:~0,1%" equ "/" set "explorer.sftm=%sst.errorlevel%"
if defined explorer.sftm call sftm /dir "%explorer.sftm%" > nul
if "%sftm.errorlevel%" equ "Folder not found" set explorer.sftm=
if "%sst.errorlevel:~0,3%" equ "~:\" cd "%sst.cd%\%sst.errorlevel:~3%"
if "%sst.errorlevel:~0,4%" equ "usr\" cd "%sst.userprofile%\%sst.errorlevel:~4%"
if "%sst.errorlevel:~0,5%" equ "root\" cd "%sst.root%"
if "%sst.errorlevel:~0,8%" equ "desktop\" cd "%sst.userprofile%\desktop\%sst.errorlevel:~8%"
goto reload
:dirup
if defined explorer.sftm (set explorer.sftm=
) else cd ..
:reload
set "explorer.dir=~:!cd:~%sst.sdir%!"
if "%cd%" equ "%sst.root%" set explorer.dir=root\
if defined explorer.sftm set explorer.dir=%explorer.sftm%
...
if "%cd%" neq "%sst.root%" (
	if defined explorer.sftm (
		call fdcore /newsession explorer %explorer.fdc.X% %explorer.fdc.Y% %explorer.fdc.W% %explorer.fdc.H% 20 6
		for /f "delims=" %%a in ('call sftm /dir "%explorer.sftm%"') do call :settile "%explorer.sftm%/%%a" /sftm
	) else (
		call fdcore /newsession explorer %explorer.fdc.X% %explorer.fdc.Y% %explorer.fdc.W% %explorer.fdc.H% 20 6
...
:settile
if "%cd%" neq "%sst.root%" (
	if "%~2" equ "/sftm" (call fdcore /addtile explorer "%~nx1" "%~x1" "%~1"
	) else call fdcore /addtile explorer "%~nx1" "%~x1" "%~f1"
) else (
...
call batbox /c 0xbf /g %explorer.X% %explorer.Y% /d " Reload " /c 0x3f /a 32 /a 30 /a 32 /c 0x%sst.window.TIcolor%%sst.window.TTcolor% /d " %explorer.dir% " /c 0xcf /g %explorer.btn.exit% %explorer.Y% /d " X "
...
if defined fdc.errorlevel.path if defined explorer.sftm (
	call :attrib "%fdc.errorlevel.path%" "%sst.errorlevel%"
	if defined explorer.reload (
		set explorer.reload=
		goto reload
	)
	if "%explorer.sftm%" neq "!explorer.sftm!" goto reload
) else if exist "%fdc.errorlevel.path%" (
	call :attrib "%fdc.errorlevel.path%" "%sst.errorlevel%"
	if defined explorer.reload (
		set explorer.reload=
		goto reload
...
if "%sst.errorlevel%" neq "OPTIONS" (
	...
	if defined explorer.sftm (
		...
		if not defined sftm.errorlevel set "explorer.sftm=%~1"
	)
) 
if "%sst.errorlevel%" neq "OPTIONS" (
	for /f "tokens=2,3" %%a in ('type "%sst.dir%\settings.cwt" ^| find "[4\"') do if "!fdc.errorlevel.icon!" equ "%%~a" (
		if defined explorer.sftm (call %%b "!fdc.errorlevel.path!" /sftm
		) else call %%b "!fdc.errorlevel.name!"
		call setres
	)
) else (
	...
	if "!sst.mvc.crrX!" gtr "!sst.desktopoptions.boxX!" if "!sst.mvc.crrX!" lss "!sst.desktopoptions.endX!" if "!sst.mvc.crrY!" equ "!sst.desktopoptions.delete!" if "%cd%" neq "%sst.root%" (
		if defined explorer.sftm (
			for %%a in (
				"title=SSTFS - Confirm deletion"
				"args=/buttons"
				"line2=Are you sure you want to delete"
				"line3='%fdc.errorlevel.name%'"
				"line5=This file is located inside the"
				"line6=system memory/RAM, witch means that it"
				"line7=will be restored after restart, but if"
				"line8=a program is currently using it, this"
				"line9=action might cause the program to"
			) do set "sst.window.%%~a"
			set "sst.window.line10=crash. Are you sure?"
			set sst.window.buttons="No" "Yes"
			call window
			if "!sst.errorlevel!" equ "1" (
				call sftm /erase "%fdc.errorlevel.path%" /s
				if defined sftm.errorlevel (
					for %%a in (
						"title=SSTFS - Confirm deletion"
						"line2=Something went wrong:"
						"line3=!sftm.errorlevel!"
					) do set "sst.window.%%~a"
					call window
				)
