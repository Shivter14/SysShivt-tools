...
if not defined sst.build (
	echo.This program reqires to be run in SysShivt tools 3.2
...
for %%a in (
	"mti=/sst.dir/assets/pack.mti 39 8 16 /sftm"
	"width=44" "height=11" "title=SysShivt tools %sst.ver%" "args=/buttons"
	"line7=      Welcome to SysShivt tools."
) do set sst.window.%%~a
set sst.window.buttons="Install" "Exit" "Enter recovery mode"
call window.cmd rem
...
if "%sst.errorlevel%" equ "CLOSED" (
	call setres
	call cmdwiz delay 500
	call shutdown /restart 1
)
if "%sst.errorlevel%" equ "1" call shutdown /restart 1
if "%sst.errorlevel%" equ "2" goto recovery
if "%sst.errorlevel%" neq "0" goto main
goto \recovery
:recovery
call setres
for %%a in (
...
if "%sst.errorlevel%" equ "49" (
	...
	for /f "tokens=2 delims==" %%a in ('set sst.errorlevel') do for %%b in ("C" "D" "E" "F" "G" "H" "I" "J") do if "%%~a" equ "%%~b" (
		...
		for %%c in ("width=30" "height=5" "title=SysShivt tools recovery" "args=/buttons"
			"line1=The boot menu of drive %%~b" "line2=was repaired successfully."
		) do set sst.window.%%~c
...
if "%sst.errorlevel%" equ "51" (
	if exist "%sst.dir%\settings.cwt" (
		...
		for %%a in ("height=5" "title=SysShivt tools recovery" "args=/buttons"
			"line2=File settings.cwt was not found." "line3=File settings.cww will be displayed instead."
		) do set sst.window.%%~a
		set sst.window.buttons="OK"
		call window rem
...
:fail.ssvm
for %%a in (
	"title=SysShivt tools Setup - Error" "args=/buttons"
	"line2=Failed to begin the instalation:" "line3=  System is not running in SSVM mode."
) do set "sst.window.%%~a"
set sst.window.buttons="Shut down"
call window.cmd rem
call shutdown
:\fail.ssvm
for %%a in (
