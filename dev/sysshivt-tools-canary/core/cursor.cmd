...
if "%~1" equ "/k" (
	if %sst.errorlevel% geq 65 if %sst.errorlevel% leq 122 goto char
	if %sst.errorlevel% geq 43 if %sst.errorlevel% leq 58 goto char
	for %%a in ("32" "8" "27") do if "%sst.errorlevel%" equ "%%~a" goto char
	if "%sst.errorlevel%" equ "9" set sst.errorlevel=OPTIONS
) else if "%sst.errorlevel%" equ "32" set sst.errorlevel=OPTIONS
...
:char
(
	set sst.errorlevel=CHAR
	call cmdwiz showcursor 1 25
	exit /b %sst.errorlevel%
)
:exit.shell
set sst.mvc.dir=%cd%
cd "%sst.dir%"
...
:mtc
call mtc.cmd
set mtc.errorlevel=%errorlevel%
if "%mtc.errorlevel%" equ "1" set sst.errorlevel=OPTIONS
:exit
call cmdwiz showcursor 1 25
exit /b 0