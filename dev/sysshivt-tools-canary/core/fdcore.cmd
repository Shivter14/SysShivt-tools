...
for /f "tokens=2 delims=$" %%a in ('set fdc.[%fdc.session%]') do set %%a
if "%~4" neq "/hideicons" for /l %%y in (0 1 %fdcs.gridY%) do for /l %%x in (0 1 %fdcs.gridX%) do if defined fdcs.[%%xx%%y] (
...
for /l %%y in (0 1 !fdc.[%fdc.session%]$fdcs.gridY!) do for /l %%x in (0 1 !fdc.[%fdc.session%]$fdcs.gridX!) do if defined fdc.[%fdc.session%]$fdcs.[%%xx%%y] (
	if !fdc.[%fdc.session%]$fdcs.[%%xx%%y].X! leq %~3 if !fdc.[%fdc.session%]$fdcs.[%%xx%%y].end! geq %~3 if !fdc.[%fdc.session%]$fdcs.[%%xx%%y].Y! leq %~4 if !fdc.[%fdc.session%]$fdcs.[%%xx%%y].nameY! geq %~4 (
		set fdc.errorlevel.name=!fdc.[%fdc.session%]$fdcs.[%%xx%%y].name!
		set fdc.errorlevel.path=!fdc.[%fdc.session%]$fdcs.[%%xx%%y].path!
		set fdc.errorlevel.icon=!fdc.[%fdc.session%]$fdcs.[%%xx%%y].icon!
		set fdc.errorlevel=True
	)
)
(
	endlocal
	set fdc.errorlevel.name=%fdc.errorlevel.name%
	set fdc.errorlevel.path=%fdc.errorlevel.path%
	set fdc.errorlevel.icon=%fdc.errorlevel.icon%
	set fdc.errorlevel=%fdc.errorlevel%
)
:\touch
:end