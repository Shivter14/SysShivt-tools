...
set /a sst.wallpaper.X=%sst.crrresX%-%sst.wallpaper.W%
set /a sst.wallpaper.X=%sst.wallpaper.X%/2
...
if exist "%sst.dir%\wallpapers\%sst.wallpaper.name%_%sst.wallpaper.type%.cww" (
	if %sst.wallpaper.X% equ 0 (type "%sst.dir%\wallpapers\%sst.wallpaper.name%_%sst.wallpaper.type%.cww"
	) else for /f "delims=" %%a in ('type "%sst.dir%\wallpapers\%sst.wallpaper.name%_%sst.wallpaper.type%.cww"') do echo.[%sst.wallpaper.X%C%%a
	goto exit
)
